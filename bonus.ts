...
function rec(temp: Weather) {
  const { data } = temp;

  const minTemperature = Math.min(
    ...data.map((element) => element.temperature_min)
  );
  const maxTemperature = Math.max(
    ...data.map((element) => element.temperature_max)
  );

  return (
    "Niedrigriste Teparatur: " +
    minTemperature +
    " °C " +
    "\n" +
    "Höchste Temparatur: " +
...
const recordTemperatures = rec(weather);
console.log("Rekordtemperaturen:", recordTemperatures);
